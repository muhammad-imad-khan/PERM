        ...
        try
        {
            using (AggregateCatalog aggregateCatalog = new AggregateCatalog())
            {
                ...
                using (CompositionContainer componsitionContainer = new CompositionContainer(aggregateCatalog))
                {
                    ...
                    IEnumerable<IEntityResolver> modules = exports.Select(export => export.Value as IEntityResolver).Where(m => m != null);

                    foreach (IEntityResolver module in modules)
                        module.SetUp(modelBuilder);
                }
            }
        }
